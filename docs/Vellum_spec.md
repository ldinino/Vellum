...
**Scope:** Runs on the current page only (it does not scan across pages or notebooks in the background), and **never on code** — code blocks and inline `code` are excluded from the text sent to Harper (they would only waste cycles). Grammar and spelling can each be turned off **independently, per scope** — an individual page, a whole section, or an entire notebook — resolving **most-specific-wins**: a page's preference beats its section's, which beats its notebook's, defaulting to on. The **Settings → Proofing** master toggles are a hard global switch (when a category is off there, no scope can turn it back on). Controls live in **Tools ▸ Proofread** (a This Page / This Section / This Notebook submenu, each with independent **Grammar** and **Spelling** toggles); and — so a page with no underlines never looks like the checker is broken — an ambient badge appears in the editor toolbar when a scope has quieted the open page, as a one-click button that turns all proofreading back on for just that page.
...
> - **Scoped proofreading (execution-plan #5, as built).** Grammar and spelling are **independent per scope**, each a tri-state — absent/`NULL` = inherit, `1` = on, `0` = off — stored as `grammar_pref` / `spell_pref` columns on the `sections` and `pages` tables (migration 7; the first cut's combined `proofing_suppressed` column from migration 6 is left as a dead column, since a dev DB may already sit at that version) and as `grammarPref` / `spellPref` on each notebook's `notebooks.json` registry entry (notebooks have no DB metadata row). Harper is context-free, so **no engine change was needed**: the effective state for the open page is resolved on the frontend **most-specific-wins** (page ▸ section ▸ notebook, defaulting to on) under the global master (`resolveProofing` in [src/lib/proofing.ts]; the `proofing` selector in `vellum.tsx`), and feeds `PageEditor`'s existing `runGrammar` toggles (both categories off ⇒ lints cleared; the re-lint effect re-runs when either resolves differently). **Code is never linted** — `extractText` skips `codeBlock` nodes and inline `code` runs. Setters `set_page_proofing` / `set_section_proofing` / `set_notebook_proofing` take both prefs and deliberately do **not** bump `updated_at` (metadata, not a content edit). The **Tools ▸ Proofread** submenu (This Page / Section / Notebook, each with Grammar + Spelling toggles that disable when their global master is off) is built by `buildProofreadMenu`; the toolbar badge is a plain one-click button that sets the open page's prefs on to re-enable everything for just that page. Setting a scope is **authoritative over its descendants** — writing a section's prefs clears its pages' own overrides, and writing a notebook's clears all its sections' and pages' overrides (`clear_all_proofing_prefs`) — so a stale per-page override (e.g. one set via the badge) can't linger and shadow a later section/notebook change.
...
- Installer: Tauri NSIS bundler, per-user install (`installMode: "currentUser"`, no admin elevation). No code signing — SmartScreen warning accepted for v1. — **done** (`bundle.targets: ["nsis"]`, `createUpdaterArtifacts: true`, `bundle.windows.nsis.installMode: "currentUser"`)
- In-app updates: `tauri-plugin-updater` against GitHub Releases (`tauri-action` builds, signs with local minisign key, uploads artifacts + `latest.json`) — **done** (see design note)
...
> - **Installer + in-app updates (as built).** NSIS per-user installer (`bundle.targets: ["nsis"]`, `bundle.windows.nsis.installMode: "currentUser"`, no admin elevation) with `bundle.createUpdaterArtifacts: true`; `tauri-plugin-updater` + `tauri-plugin-process` are registered in `lib.rs` and granted `updater:default` / `process:allow-restart` in `capabilities/default.json`. `UpdaterProvider` (`src/state/updater.tsx`, mounted inside `ActiveEditorProvider`) runs one silent `check()` ~4 s after launch — gated to packaged release builds (`import.meta.env.PROD`) so `tauri dev` stays quiet — against the GitHub `latest.json` endpoint, downloads any newer version in the background, and raises a non-blocking `UpdateNotice` ("Restart now / Later"); **Settings ▸ About** runs the same flow on demand and mirrors its status. Windows force-quits the app the instant the installer runs, so `applyUpdate()` flushes the open page (`flushSaves` + inline-image sweep, the same refs the close path uses, under one timeout cap) **before** `install()`, then `relaunch()`. Releases are built/signed/drafted by `.github/workflows/release.yml` (tauri-action, Windows-only, on a `vX.Y.Z` tag); `scripts/bump-version.ps1` keeps the three version files in sync and `scripts/release-notes.ps1` renders the GitHub release body from CHANGELOG.md (Added/Changed → "What's New", Fixed → "Fixed", plus an install/disclaimer footer). **Maintainer go-live (one-time):** `tauri signer generate` → paste the public key into `plugins.updater.pubkey`, add `TAURI_SIGNING_PRIVATE_KEY` (+ `…_PASSWORD`) repo secrets, push the tag, then publish the draft release (the updater only sees published, non-draft releases).
