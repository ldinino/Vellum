...
- Styling is bespoke retro CSS; no CSS framework. Tokens live in `src/styles/tokens.css`. Four root attributes drive appearance: `data-chrome` (`aero` | `98`) picks the window-chrome family, `data-theme` picks the colour scheme within it (`light`/`dark`/`oled`, or `98-standard`/`98-dark`/`98-eggplant`/`98-spruce`/`98-rose`/`98-desert`/`98-storm`), `data-scheme` (`light` | `dark`) is what all dark styling and `color-scheme` key on, and `data-corners` (`rounded` | `square`) is a global user setting. All four are written by `applyAppearance` in `src/state/vellum.tsx`. The Windows 98 family lives entirely in `src/styles/theme98.css`. 7.css and Office-Ribbon-2010 were measured colour references only — 7.css is no longer imported (it was scoped to `.win7` wrappers the app never had), though its values remain in the tokens and it is still credited.
